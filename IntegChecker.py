...
import sys 							#import system module for handling system calls
...
#Metascan API key
GLOB_APIKEY = 'ADD HERE'
  
#Metascan API hash lookup URL                   
GLOB_HASHURL = 'http://hashlookup.metascan-online.com/v2/hash/'
#Metascan API hash/file result URL
GLOB_HASHRESULTURL = 'http://metascan-online.com/en/scanresult/file/'  
#Metascan API file scan upload URL
GLOB_UPURL = 'http://scan.metascan-online.com/v2/file'        
...
##Returns: string/
#############################################################################################
def updateFile(data1, data2, data3, data4, data5):
	##
	##README
	##
	...
	replace('C:\Python27\Visual\index.html.tmp', "data-actual=0.0 indicator-widget></div><!--rep1-->", "data-actual=" + str(data1) + " indicator-widget></div><!--rep1-->")
	replace('C:\Python27\Visual\index.html.tmp', "data-actual=0.0 indicator-widget></div><!--rep2-->", "data-actual=" + str(data2) + " indicator-widget></div><!--rep2-->")
	replace('C:\Python27\Visual\index.html.tmp', "data-actual=0.0 indicator-widget></div><!--rep3-->" , "data-actual=" + str(data3) + " indicator-widget></div><!--rep2-->")
	replace('C:\Python27\Visual\index.html.tmp', "data-actual=0.0 indicator-widget></div><!--rep4-->", "data-actual=" + str(data4) + " indicator-widget></div><!--rep4-->")
	replace('C:\Python27\Visual\index.html.tmp', "data-actual=0.0 indicator-widget></div><!--rep5-->", "data-actual=" + str(data5) + " indicator-widget></div><!--rep5-->")
...
def doMath():
	...
	integrity_data = (float(glob_c) + float(glob_e) + float(glob_u)) / (float(fileCount))
	cleaned_data = (float(glob_c) + float(glob_e))	/ (float(fileCount))
	infected_data = (float(glob_i)) / (float(fileCount))
	suspicious_data = (float(glob_s)) / (float(fileCount))
	unknown_data= (float(glob_u)) / (float(fileCount))
	updateFile(integrity_data,cleaned_data,infected_data,suspicious_data,unknown_data)
	# print str(glob_c) +"\n"
	# print str(glob_s) +"\n"
	# print str(glob_i) +"\n"
	# print str(glob_u) +"\n"
	...
	# print str(integrity_data) +"\n"
	# print str(cleaned_data) +"\n"
	# print str(infected_data) +"\n"
	# print str(suspicious_data) +"\n"
	# print str(unknown_data) +"\n"

	print "\n[*] Finished walking directory.\n[*] Goodbye!"
	sys.exit(0)
...
#############################################################################################
##Function: updateCSV
##Purpose: updates the tree diagram visual file
##Returns: none
#############################################################################################
def updateCSV():

	f = open('C:\Python27\Infected_Files.log', 'r')
	out = open('C:\Python27\Visual\output.csv', 'w')
	...
	lines = open('C:\Python27\Visual\output.csv', 'r').readlines()
...
def main():
	...
	rootDir = glob_dir 
	print ("\n" + "="*75)
	print ("[*]Starting at directory: " + rootDir)
	print ("="*75+ "\n")
	time.sleep(5)
	glob_e = 0	#u for uknown
